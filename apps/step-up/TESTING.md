...
- Test studios are omitted from `/login` and `/register`. Use `?includeTest=1` to show them.
- Analytics demo studio (manual QA of Payments / Retention / funnel / batch revenue): `pnpm --filter @step-up/api prisma:seed:analytics` → sign in as `soundhar.adi+analytics-owner@gmail.com` (`studio-analytics-1`)
