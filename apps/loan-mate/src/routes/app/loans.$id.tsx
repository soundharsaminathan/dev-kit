...
import { LoanReviewScreen } from "@/modules/loans/loan-review-screen";
...
function LoanDetailPage() {
  ...
  const reject = useMutation({
    mutationFn: (reason: string) => api.post(`/loans/${id}/reject`, { reason }),
    onSuccess: invalidate,
    onError,
  });
  ...
  return (
    <LoanReviewScreen
      loan={l}
      canApprove={
        l.status === "VERIFIED" && roleAllowed(user?.role, LOAN_APPROVE_ROLES)
      }
      canReject={
      ...
      error={error}
      message={msg}
      onBack={() => void navigate({ to: "/app/loans" })}
      onSchedule={() =>
        void navigate({
      ...
      onSubmit={l.status === "DRAFT" ? () => submit.mutate() : undefined}
      onVerify={l.status === "SUBMITTED" ? () => verify.mutate() : undefined}
      onRequestApproval={
        l.status === "VERIFIED" ? () => requestApproval.mutate() : undefined
      }
      documentsPanel={<DocumentsPanel entityType="LOAN" entityId={l.id} />}
      history={
              ...
              <div className="lm-card lm-table-wrap">
                ...
                {installments.length === 0 ? (
                  <p className="lm-muted">
                    Schedule is generated at disbursement.
                  </p>
                ) : (
